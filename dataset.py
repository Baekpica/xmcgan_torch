...
_bert_layer = BertModel.from_pretrained('bert-base-uncased')
# _bert_layer = torch.nn.DataParallel(_bert_layer)
# _bert_layer = _bert_layer.cuda()
_bert_layer.eval()


def get_bert_for_caption(captions: List[str], max_text_length: int = 17):
    ...
    with torch.no_grad():
        token_embedding = _bert_layer(ids, attention_mask=input_mask, token_type_ids=segment_ids)
...
for set_name in set_list:
    ...
    for idx, (images, captions) in enumerate(test_loader):
        # print(images)
        # print(captions)
        t1, t2, t3 = get_bert_for_caption(captions)
