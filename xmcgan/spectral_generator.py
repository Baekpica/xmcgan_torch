...
class SelfModulationBatchNorm(nn.Module):
    def __init__(self, in_ch_dim, cond_dim):
        ...
        self.gamma = utils.spectral_norm(nn.Linear(cond_dim, in_ch_dim))
        self.beta = utils.spectral_norm(nn.Linear(cond_dim, in_ch_dim))
...
class ResBlockUp(nn.Module):
    def __init__(self, in_ch_dim, out_ch_dim, size, cond_dim):
        ...
        self.main_flow_1 = nn.Sequential(
            nn.ReLU(),
            nn.Upsample(size*2),
            utils.spectral_norm(nn.Conv2d(in_ch_dim, out_ch_dim, kernel_size=(3, 3), padding=1, stride=(1, 1)))
        )
        self.bn2 = SelfModulationBatchNorm(out_ch_dim, cond_dim)
        self.main_flow_2 = nn.Sequential(
            nn.ReLU(),
            utils.spectral_norm(nn.Conv2d(out_ch_dim, out_ch_dim, kernel_size=(3, 3), padding=1, stride=(1, 1)))
        )
...
class AttnSelfModulationBatchNorm(nn.Module):
    def __init__(self, in_ch_dim, cond_dim):
        ...
        self.in_ch_dim = in_ch_dim
        self.cond_dim = cond_dim
        self.gamma = utils.spectral_norm(nn.Conv2d(cond_dim, in_ch_dim, kernel_size=(1, 1), stride=(1, 1)))
        self.beta = utils.spectral_norm(nn.Conv2d(cond_dim, in_ch_dim, kernel_size=(1, 1), stride=(1, 1)))
        self.bn = nn.BatchNorm2d(in_ch_dim)
...
class Generator(nn.Module):
    def __init__(self, batch_size):
        ...
        self.linear1 = utils.spectral_norm(nn.Linear(768, 128))
        self.linear2 = utils.spectral_norm(nn.Linear(256,4*4*16))
        self.res_up1 = ResBlockUp(16, 16, 4, 256)
        self.res_up2 = ResBlockUp(16, 8, 8, 256)
        self.conv1 = utils.spectral_norm(nn.Conv2d(8, 768, kernel_size=(1, 1), stride=(1, 1)))
        ...
        self.conv2 = utils.spectral_norm(nn.Conv2d(1, 3, kernel_size=(3, 3), padding=1, stride=(1, 1)))
